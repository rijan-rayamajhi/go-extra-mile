...
class VehicleFirestoreDataSource {
  ...
  Future<String> uploadVehicleImage(String vehicleId, String userId, File imageFile, String fieldName) async {
    try {
      //upload to firebase storage
      final url = await _storage.uploadFile(file: imageFile, path: 'vehicles/$userId/$vehicleId/$fieldName'); 
     ...
     if (fieldName == 'vehicleSlideImages') {
      await _getUserVehiclesCollection(userId).doc(vehicleId).update({fieldName: FieldValue.arrayUnion([url])});
     } else {
      await _getUserVehiclesCollection(userId).doc(vehicleId).update({fieldName: url});
     }
...
Future<int> getTotalNotVerifiedVehicles( String userId) async {
  try {
    ...
    // For each user, query their vehicles subcollection
        final vehiclesSnapshot = await _getUserVehiclesCollection(userId)
          .where('verificationStatus', isEqualTo: 'notVerified')
          .get();
